...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	// Use camera->SetPosition() and camera->SetRotation(glm::quat(euler_angles)) 
	{
		...
		glm::vec3 mirrorToCam = glm::normalize(camPosition - mirrorPos);
		glm::vec3 R = glm::reflect(mirrorToCam, glm::vec3(1, 0, 0));
		...
		camera->View = glm::lookAt(mirrorPos, mirrorPos + R, glm::vec3(1, 0, 0));
		camera->SetPosition(mirrorToCam);
		//camera->SetRotation(R);
